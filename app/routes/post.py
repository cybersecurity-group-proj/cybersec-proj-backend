...
from app.auth.dependencies import access_token_bearer, PermissionChecker , get_current_user, get_current_user_with_permissions
...
async def update_post(post_id: str, updated_post : PostUpdate, session: db_session,  user_details: access_token_bearer, permission: edit_permission_checker, user: get_current_user_with_permissions):    
    post = await repo.get_post(post_id, session)
